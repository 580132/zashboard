...
// Local to Tools: the shared useKeyboardInset composable was removed upstream
// in favor of `interactive-widget=resizes-content`, but the symbol bar still
// needs a reactive signal for "keyboard is up".
const keyboardInset = ref(0)
const updateKeyboardInset = () => {
  const viewport = window.visualViewport
  if (!viewport) return
  const height = window.innerHeight - viewport.height - viewport.offsetTop
  keyboardInset.value = height > 1 ? Math.round(height) : 0
}
onMounted(() => {
  const viewport = window.visualViewport
  if (!viewport) return
